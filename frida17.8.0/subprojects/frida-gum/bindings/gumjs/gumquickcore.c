                      ...
                      GumScriptScheduler * scheduler)
{
  ...
  self->native_callbacks = g_hash_table_new_full (NULL, NULL, NULL,
      (GDestroyNotify) gum_quick_native_callback_finalize);
  ...
  JS_DefinePropertyValueStr (ctx, ns, "Sunda", obj, JS_PROP_C_W_E);
...
_gum_quick_core_dispose (GumQuickCore * self)
{
  ...
  g_hash_table_remove_all (self->native_callbacks);
...
GUMJS_DEFINE_CONSTRUCTOR (gumjs_native_callback_construct)
{
  ...
  cb->wrapper = JS_DupValue (ctx, wrapper);
  cb->func = JS_DupValue (ctx, func);
  cb->core = core;
  ...
  JS_DefinePropertyValue (ctx, wrapper,
      ...
      0);
  g_hash_table_add (core->native_callbacks, cb);
...
{
  JS_SetOpaque (callback->wrapper, NULL);
  JS_FreeValue (callback->core->ctx, callback->wrapper);
  JS_FreeValue (callback->core->ctx, callback->func);
...
# endif
  ...
  /*
   * Pin the callback function while this closure is in flight. The wrapper can
   * disappear independently of the underlying function object under hot attach
   * pressure, and pinning the wrapper here was the direct crash site in the
   * reduced bilipai repro.
   */
  JS_DupValue (ctx, self->func);
