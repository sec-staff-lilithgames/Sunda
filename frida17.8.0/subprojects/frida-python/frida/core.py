...
(function () {
  function defineBridge(name) {
    ...
    Object.defineProperty(globalThis, name, {
      ...
      get: () => {
        let bridge;
        send({ type: "sunda:load-bridge", name });
        recv("sunda:bridge-loaded", message => {
...
def _find_bridge_dir() -> Optional[Path]:
    global _bridge_dir
    ...
    bridge_dir = Path(frida_tools.__file__).resolve().parent / "bridges"
    _bridge_dir = bridge_dir if bridge_dir.is_dir() else None
    return _bridge_dir


def _inject_bridge_loader(source: str) -> str:
    if _find_bridge_dir() is None:
        return source
    return _BRIDGE_LOADER_TEMPLATE + "\n" + source
...
class Script:
    ...
    def _send_rpc_call(self, request_id: int, args: Any, data: Optional[bytes]) -> None:
        self.post(["sunda:rpc", request_id, *args], data)
    ...
    def _on_message(self, raw_message: str, data: Optional[bytes]) -> None:
        ...
        elif mtype == "send" and isinstance(payload, list) and len(payload) > 0 and payload[0] == "sunda:rpc":
            request_id = payload[1]
            operation = payload[2]
            params = payload[3:]
            self._on_rpc_message(request_id, operation, params, data)
        elif self._try_handle_bridge_request(message):
            pass
        else:
    ...
    def _try_handle_bridge_request(self, message: Mapping[str, Any]) -> bool:
        ...
        if payload.get("type") != "sunda:load-bridge":
            return False
        ...
        stem = str(payload["name"]).lower()
        try:
            bridge = next(p for p in bridge_dir.glob("*.js") if p.stem == stem)
        except StopIteration:
            return False

        self.post(
            {
                "type": "sunda:bridge-loaded",
...
class Session:
    ...
    def create_script(
        ...
        if snapshot is None:
            source = _inject_bridge_loader(source)
