...
def compile_bridges(inputs: List[Path], output_dir: Path, priv_dir: Path, npm: Path):
    ...
    for srcfile in inputs:
        ...
        if "node_modules" in subpath.parts:
            patched_dependency_files.append((srcfile, dstfile))
    ...
    subprocess.run([npm, "ci", "--no-audit", "--no-fund"], **run_kwargs)

    for srcfile, dstfile in patched_dependency_files:
        shutil.copy(srcfile, dstfile)
