package cm;

import android.content.Context;
import android.text.TextUtils;
import com.google.android.gms.ads.AdSize;
import com.google.android.gms.ads.MobileAds;
import com.google.android.gms.ads.VersionInfo;
import com.google.android.gms.ads.admanager.AdManagerAdView;
import io.bidmachine.AdsFormat;
import zl.l;
import zl.s;
import zl.t;

/* compiled from: r8-map-id-329fa88356c9e071bedcd87e8cbc16a4cb0c028932e63902b8210957806638ae */
/* loaded from: classes9.dex */
public class b extends am.a {
    public b() {
        super(19, new vr.a(22, 0, 0));
    }

    @Override // am.a, am.b
    public t createAd(l lVar, AdsFormat adsFormat, s sVar) {
        switch (a.f12376a[adsFormat.ordinal()]) {
            case 1:
            case 2:
                return new f(lVar, adsFormat, sVar, AdSize.BANNER);
            case 3:
                return new f(lVar, adsFormat, sVar, AdSize.LEADERBOARD);
            case 4:
                return new f(lVar, adsFormat, sVar, AdSize.MEDIUM_RECTANGLE);
            case 5:
            case 6:
            case 7:
                return new h(lVar, adsFormat, sVar);
            case 8:
            case 9:
            case 10:
                return new k(lVar, adsFormat, sVar);
            default:
                return null;
        }
    }

    @Override // am.a, am.b
    public vr.a getVersion() {
        VersionInfo version = MobileAds.getVersion();
        return new vr.a(version.getMajorVersion(), version.getMinorVersion(), version.getMicroVersion());
    }

    @Override // am.a, am.b
    public void initialize(Context context) {
        MobileAds.initialize(context);
    }

    @Override // am.a, am.b
    public boolean isGAMPresent(Context context) {
        boolean z10;
        try {
        } catch (Exception unused) {
        }
        if (context.getPackageManager().getApplicationInfo(context.getPackageName(), 128).metaData == null ? false : !TextUtils.isEmpty(r5.getString("com.google.android.gms.ads.APPLICATION_ID"))) {
            try {
                int i10 = AdManagerAdView.f28828b;
                z10 = true;
            } catch (Exception unused2) {
                z10 = false;
            }
            if (z10) {
                return true;
            }
        }
        return false;
    }
}
