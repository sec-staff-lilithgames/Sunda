...
public abstract class e {
    ...
    public static double b(double d10) {
        ...
        return ((f.signum(d10) * 400.0d) * dPow) / (dPow + 27.13d);
    }

    public static double c(double[] dArr) {
        double[] dArrMatrixMultiply = f.matrixMultiply(dArr, f70957a);
        double dB = b(dArrMatrixMultiply[0]);
        double dB2 = b(dArrMatrixMultiply[1]);
        double dB3 = b(dArrMatrixMultiply[2]);
        return Math.atan2(((dB + dB2) - (dB3 * 2.0d)) / 9.0d, ((((-12.0d) * dB2) + (dB * 11.0d)) + dB3) / 11.0d);
    }

    public static double d(double d10) {
        double dAbs = Math.abs(d10);
        return Math.pow(Math.max(0.0d, (27.13d * dAbs) / (400.0d - dAbs)), 2.380952380952381d) * f.signum(d10);
    }
    ...
    public static b solveToCam(double d10, double d11, double d12) {
        return b.fromInt(solveToInt(d10, d11, d12));
    }
    ...
    public static int solveToInt(double d10, double d11, double d12) {
        ...
        if (d11 < 1.0E-4d || d12 < 1.0E-4d || d12 > 99.9999d) {
            return c.argbFromLstar(d12);
        }
        double dSanitizeDegreesDouble = (f.sanitizeDegreesDouble(d10) / 180.0d) * 3.141592653589793d;
        double dYFromLstar = c.yFromLstar(d12);
        double dSqrt = Math.sqrt(dYFromLstar) * 11.0d;
        g gVar = g.f70961k;
        double dPow = 1.0d / Math.pow(1.64d - Math.pow(0.29d, gVar.getN()), 0.73d);
        double d17 = 2.0d;
        double dCos = (Math.cos(dSanitizeDegreesDouble + 2.0d) + 3.8d) * 0.25d * 3846.153846153846d * gVar.f70966e * gVar.f70964c;
        ...
        while (true) {
            ...
            dArr = f70959c;
            if (i11 >= 5) {
                d14 = dSanitizeDegreesDouble;
                i10 = 1;
                break;
            }
            double d18 = dCos;
            double d19 = dSqrt / 100.0d;
            i10 = 1;
            d14 = dSanitizeDegreesDouble;
            double dPow2 = Math.pow(((d11 == 0.0d || dSqrt == 0.0d) ? 0.0d : d11 / Math.sqrt(d19)) * dPow, 1.1111111111111112d);
            double dPow3 = (Math.pow(d19, (1.0d / gVar.f70965d) / gVar.f70971j) * gVar.getAw()) / gVar.getNbb();
            double d20 = (((0.305d + dPow3) * 23.0d) * dPow2) / (((108.0d * dPow2) * dSin) + (((dPow2 * 11.0d) * dCos2) + (23.0d * d18)));
            double d21 = d20 * dCos2;
            double d22 = d20 * dSin;
            double d23 = dPow3 * 460.0d;
            dArrMatrixMultiply = f.matrixMultiply(new double[]{d(((288.0d * d22) + ((451.0d * d21) + d23)) / 1403.0d), d(((d23 - (891.0d * d21)) - (261.0d * d22)) / 1403.0d), d(((d23 - (d21 * 220.0d)) - (d22 * 6300.0d)) / 1403.0d)}, f70958b);
            double d24 = dArrMatrixMultiply[0];
            if (d24 < 0.0d) {
                break;
            }
            double d25 = dArrMatrixMultiply[1];
            if (d25 < 0.0d) {
                break;
            }
            double d26 = dArrMatrixMultiply[2];
            if (d26 < 0.0d) {
                break;
            }
            ...
            double d28 = d27 - dYFromLstar;
            if (Math.abs(d28) < 0.002d) {
                break;
            }
            dSqrt -= (d28 * dSqrt) / (d27 * d13);
            i11++;
            d17 = d13;
            dCos = d18;
            dSanitizeDegreesDouble = d14;
        }
        int iArgbFromLinrgb = (dArrMatrixMultiply[0] > 100.01d || dArrMatrixMultiply[1] > 100.01d || dArrMatrixMultiply[2] > 100.01d) ? 0 : c.argbFromLinrgb(dArrMatrixMultiply);
        ...
        while (i12 < 12) {
            ...
            double d32 = dArr[i10];
            double d33 = dArr[2];
            double d34 = i12 % 4 <= i10 ? 0.0d : 100.0d;
            double d35 = i12 % 2 == 0 ? 0.0d : 100.0d;
            if (i12 < 4) {
                double d36 = ((dYFromLstar - (d32 * d34)) - (d33 * d35)) / d31;
                dArr2 = e(d36) ? new double[]{d36, d34, d35} : new double[]{-1.0d, -1.0d, -1.0d};
            } else if (i12 < 8) {
                double d37 = ((dYFromLstar - (d31 * d35)) - (d33 * d34)) / d32;
                if (e(d37)) {
                    dArr3 = new double[]{d35, d37, d34};
                    dArr2 = dArr3;
                } else {
                    dArr2 = new double[]{-1.0d, -1.0d, -1.0d};
                }
            } else {
                double d38 = ((dYFromLstar - (d31 * d34)) - (d32 * d35)) / d33;
                if (e(d38)) {
                    dArr3 = new double[]{d34, d35, d38};
                    dArr2 = dArr3;
                } else {
                    dArr2 = new double[]{-1.0d, -1.0d, -1.0d};
                }
        ...
        double[][] dArr6 = {dArr4, dArr5};
        double[] dArr7 = dArr6[0];
        double dC2 = c(dArr7);
        double[] dArr8 = dArr6[1];
        for (int i14 = 0; i14 < 3; i14++) {
            double d43 = dArr7[i14];
            double d44 = dArr8[i14];
            if (d43 != d44) {
                if (d43 < d44) {
                    iCeil = (int) Math.floor(f(d43) - 0.5d);
                    dFloor = Math.ceil(f(dArr8[i14]) - 0.5d);
                } else {
                    iCeil = (int) Math.ceil(f(d43) - 0.5d);
                    dFloor = Math.floor(f(dArr8[i14]) - 0.5d);
                }
                int i15 = (int) dFloor;
                for (int i16 = 0; i16 < 8 && Math.abs(i15 - iCeil) > 1; i16++) {
                    int iFloor = (int) Math.floor((iCeil + i15) / d13);
                    double d45 = f70960d[iFloor];
                    double d46 = dArr7[i14];
                    double d47 = (d45 - d46) / (dArr8[i14] - d46);
                    double d48 = dArr7[0];
                    double d49 = ((dArr8[0] - d48) * d47) + d48;
                    double d50 = dArr7[1];
                    double d51 = ((dArr8[1] - d50) * d47) + d50;
                    double d52 = dArr7[2];
                    double[] dArr9 = {d49, d51, ((dArr8[2] - d52) * d47) + d52};
                    double dC3 = c(dArr9);
                    double d53 = dC2;
                    if (a(d53, d14, dC3)) {
                        i15 = iFloor;
        ...
        return c.argbFromLinrgb(new double[]{(dArr7[0] + dArr8[0]) / d13, (dArr7[1] + dArr8[1]) / d13, (dArr7[2] + dArr8[2]) / d13});
    }
}
