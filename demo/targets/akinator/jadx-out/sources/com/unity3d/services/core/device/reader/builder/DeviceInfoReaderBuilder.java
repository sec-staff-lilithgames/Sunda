...
public class DeviceInfoReaderBuilder {
    ...
    private JsonFlattenerRules getTsiRequestStorageRules() {
        return new JsonFlattenerRules(Arrays.asList(DtbDeviceRegistration.CONFIG_PRIVACY_KEY, InMobiSdk.IM_GDPR_CONSENT_GDPR_APPLIES, "framework", Q6.F1, "unity", "pipl", "configuration", "user", JsonStorageKeyNames.UNIFIED_CONFIG_KEY), Collections.singletonList("value"), Arrays.asList("ts", "exclude", "pii", "nonBehavioral", "nonbehavioral"));
    }
    ...
    public JsonFlattenerRules getMediationInfoRules() {
        return new JsonFlattenerRules(Collections.singletonList(DTBMetricReport.ADSERVER), Collections.singletonList("value"), Arrays.asList("ts", C3457t5.f38909a));
    }
}
