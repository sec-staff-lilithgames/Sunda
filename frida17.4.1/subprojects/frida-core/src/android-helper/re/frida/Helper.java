...
public class Helper {
	public static void main(String[] args) {
		if (args.length != 1) {
			System.err.println("Usage: sunda-helper <instance-id>");
			System.exit(1);
			return;
		}
		...
		new File("/data/local/tmp/sunda-helper-" + instanceId + ".dex").delete();

		LocalServerSocket socket;
		try {
			socket = new LocalServerSocket("/sunda-helper-" + instanceId);
		} catch (IOException e) {
