...
def main(argv):
    ...
    if host_os == "windows":
        ...
        for helper in {helper_modern, helper_legacy, helper_emulated_modern, helper_emulated_legacy}:
            ...
            # Sunda fingerprint hardening: avoid stock Frida helper asset names.
            embedded_helper = priv_dir / f"sunda-helper-{arch}.exe"
        ...
        for missing_arch in pending_archs:
            embedded_helper = priv_dir / f"sunda-helper-{missing_arch}.exe"
            embedded_helper.write_bytes(b"")
            embedded_assets += [embedded_helper]
    elif host_os in {"macos", "ios", "watchos", "tvos", "xros"}:
        embedded_helper = priv_dir / "sunda-helper"
        ...
        embedded_helper_modern = priv_dir / f"sunda-helper-64"
        embedded_helper_legacy = priv_dir / f"sunda-helper-32"
