namespace Frida {
	...
	public sealed class LinuxHostSession : LocalHostSession {
		...
		construct {
			...
			var blob32 = Frida.Data.Agent.get_sunda_agent_32_so_blob ();
			var blob64 = Frida.Data.Agent.get_sunda_agent_64_so_blob ();
			var emulated_arm = Frida.Data.Agent.get_sunda_agent_arm_so_blob ();
			var emulated_arm64 = Frida.Data.Agent.get_sunda_agent_arm64_so_blob ();
			// Sunda fingerprint hardening: avoid stock Frida agent filenames.
			agent = new AgentDescriptor (PathTemplate ("sunda-agent-<arch>.so"),
				new Bytes.static (blob32.data),
				new Bytes.static (blob64.data),
				new AgentResource[] {
					new AgentResource ("sunda-agent-arm.so", new Bytes.static (emulated_arm.data), tempdir),
					new AgentResource ("sunda-agent-arm64.so", new Bytes.static (emulated_arm64.data), tempdir),
				},
				AgentMode.INSTANCED,
				tempdir);
#endif
		...
		protected override string? get_emulated_agent_path (uint pid) throws Error {
			...
			switch (cpu_type_from_pid (pid)) {
				case Gum.CpuType.IA32:
					// Sunda fingerprint hardening: avoid stock Frida agent filenames.
					name = "sunda-agent-arm.so";
					break;
				case Gum.CpuType.AMD64:
					// Sunda fingerprint hardening: avoid stock Frida agent filenames.
					name = "sunda-agent-arm64.so";
					break;
				default:
	...
	private sealed class SystemServerAgent : InternalAgent {
...
#if ARM || ARM64
		private const LinuxSyscall SUSPEND_SAFEPOINT_BASE_MASK = RESTART
			| IOCTL
			| POLL_LIKE
			| FUTEX
			| READ
			...
			| ACCEPT
			| RECV;
		private const LinuxSyscall SUSPEND_SAFEPOINT_FALLBACK_MASK = SUSPEND_SAFEPOINT_BASE_MASK;
		...
		private async void await_syscall_for_thread (uint tid, string thread_name, Gee.Collection<uint> suspended_tids,
			...
			try {
				yield helper.await_syscall (tid, SUSPEND_SAFEPOINT_BASE_MASK, cancellable);
				suspended = true;
			} catch (GLib.Error e) {
				if (e is Error.TIMED_OUT) {
					try {
						yield helper.await_syscall (tid, SUSPEND_SAFEPOINT_FALLBACK_MASK, cancellable);
						suspended = true;
					} catch (GLib.Error retry_error) {
						if (retry_error is Error.TIMED_OUT) {
							printerr ("Timed out twice while waiting for syscall on %s thread; continuing without suspending it.\n",
								thread_name);
						} else {
							printerr ("Unable to await syscall on %s thread (%s); continuing without suspending it.\n",
								thread_name, retry_error.message);
						}
					}
				} else {
					printerr ("Unable to await syscall on %s thread (%s); continuing without suspending it.\n",
