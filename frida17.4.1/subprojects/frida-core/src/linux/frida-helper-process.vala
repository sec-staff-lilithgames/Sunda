namespace Frida {
	...
	private sealed class HelperFactory {
		...
		public async LinuxHelper obtain (Cancellable? cancellable) throws Error, IOError {
			...
			try {
				string socket_path = "/sunda-" + Uuid.string_random ();
	...
	private sealed class ResourceStore {
		...
		public ResourceStore (TemporaryDirectory tempdir) throws Error {
			...
			var blob32 = Frida.Data.Helper.get_sunda_helper_32_blob ();
			if (blob32.data.length > 0)
				// Sunda fingerprint hardening: avoid stock Frida helper filenames.
				helper32 = make_temporary_helper ("sunda-helper-32", blob32.data);

			var blob64 = Frida.Data.Helper.get_sunda_helper_64_blob ();
			if (blob64.data.length > 0)
				// Sunda fingerprint hardening: avoid stock Frida helper filenames.
				helper64 = make_temporary_helper ("sunda-helper-64", blob64.data);
#else
