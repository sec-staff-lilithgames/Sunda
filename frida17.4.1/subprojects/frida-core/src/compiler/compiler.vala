		...
		private void init () {
...
#else
			unowned uint8[] backend_so = Frida.Data.Compiler.get_sunda_compiler_backend_so_blob ().data;

			Module? backend = null;

			if (MemoryFileDescriptor.is_supported ()) {
				var fd = MemoryFileDescriptor.from_bytes ("sunda-compiler-backend.so", new Bytes.static (backend_so));
				try {
			...
			if (backend == null) {
				try {
					string name_used;
					{
						var fd = new FileDescriptor (FileUtils.open_tmp ("sunda-compiler-backend-XXXXXX.so", out name_used));
			...
			build = resolve_symbol (backend, "_sunda_compiler_backend_build");
			watch = resolve_symbol (backend, "_sunda_compiler_backend_watch");
			WatchSession.dispose = resolve_symbol (backend, "_sunda_compiler_backend_watch_session_dispose");
#endif
#endif
		}
