...
func makeFridaShimsPlugin() esbuild.Plugin {
	const (
		nsBuiltins = "sunda-builtins"
		nsShim     = "sunda-shim"
	)

	const (
		builtinFilter        = `^sunda-builtins://(.+)$`
...
func loaderFor(path string) esbuild.Loader {
	...
	default:
		panic(fmt.Sprintf("sunda-shims: unsupported file type %q", ext))
	}
}
