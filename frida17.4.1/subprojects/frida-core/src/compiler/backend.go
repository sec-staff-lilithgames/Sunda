...
//export _sunda_compiler_backend_build
func _sunda_compiler_backend_build(cProjectRoot, cEntrypoint *C.char, outputFormat C.FridaOutputFormat, bundleFormat C.FridaBundleFormat,
...
//export _sunda_compiler_backend_watch
func _sunda_compiler_backend_watch(cProjectRoot, cEntrypoint *C.char, outputFormat C.FridaOutputFormat, bundleFormat C.FridaBundleFormat,
	disableTypeCheck, sourceMap, compress uintptr, cPlatform *C.char, cExternals **C.char, numExternals C.int,
...
//export _sunda_compiler_backend_watch_session_dispose
func _sunda_compiler_backend_watch_session_dispose(h uintptr) {
	handle := cgo.Handle(h)
...
func makeContext(options BuildOptions, callbacks BuildEventCallbacks) (ctx esbuild.BuildContext, err error) {
	...
	buildOpts := esbuild.BuildOptions{
		...
		Format:            format,
		Inject:            []string{"sunda-builtins:///node-globals.js"},
...
func emitDiagnostic(category string, message esbuild.Message, onDiagnostic BuildDiagnosticCallback) {
	...
	if message.PluginName == "sunda-custom-ts" && len(message.Notes) == 2 {
