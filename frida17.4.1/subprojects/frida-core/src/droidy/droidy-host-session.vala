		...
		private async RemoteServer get_remote_server (Cancellable? cancellable) throws Error, IOError {
			...
			try {
				...
				if (connection.closed)
					// Sunda fingerprint hardening: avoid stock Frida daemon naming in user-facing strings.
					throw new Error.SERVER_NOT_RUNNING ("Unable to connect to remote sunda daemon");
				...
				if (e is IOError.CANCELLED) {
					...
					if (e is Error.SERVER_NOT_RUNNING) {
						// Sunda fingerprint hardening: avoid stock Frida daemon naming in user-facing strings.
						api_error = new Error.SERVER_NOT_RUNNING ("Unable to connect to remote sunda daemon");
					} else if (connection != null) {
						// Sunda fingerprint hardening: avoid stock Frida daemon naming in user-facing strings.
						api_error = new Error.PROTOCOL ("Incompatible sunda daemon version");
					} else {
						// Sunda fingerprint hardening: avoid stock Frida daemon naming in user-facing strings.
						api_error = new Error.SERVER_NOT_RUNNING ("Unable to connect to remote sunda daemon: %s",
							e.message);
					}
		...
		private async AndroidHelperClient get_helper_client (Cancellable? cancellable) throws Error, IOError {
			...
			try {
				...
				try {
					...
					// Sunda fingerprint hardening: avoid stock Frida helper socket names.
					yield client.request_protocol_change ("localabstract:/sunda-helper-" + instance_id, cancellable);
				} catch (GLib.Error e) {
					client.close.begin ();
					throw e;
				}
