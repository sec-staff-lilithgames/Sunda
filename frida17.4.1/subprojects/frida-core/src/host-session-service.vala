		...
		private async AgentEntry establish (uint pid, HashTable<string, Variant> options,
			...
			try {
				...
				if (e is IOError.CANCELLED && (cancellable == null || !cancellable.is_cancelled ())) {
					// Sunda fingerprint hardening: avoid stock Frida agent naming in user-facing strings.
					e = new Error.PROCESS_NOT_RESPONDING ("Process with pid %u either refused to load sunda-agent, " +
